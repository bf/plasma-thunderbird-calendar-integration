...
void EventModel::createMonitor()
{
    ...
    connect(mMonitor, &Akonadi::Monitor::itemChanged, this, [this](const Akonadi::Item &item) {
        if (!item.hasPayload<KCalendarCore::Incidence::Ptr>()) {
            qCDebug(THUNDERBIRDEVENTSPLUGIN_LOG) << "Item" << item.id() << "has no payload!";
            return;
        }
...
{
    qCDebug(THUNDERBIRDEVENTSPLUGIN_LOG) << "Populating events from collection" << col.id();
    ...
    connect(job, &Akonadi::ItemFetchJob::result, job, [this, col](KJob *job) {
        ...
        qCDebug(THUNDERBIRDEVENTSPLUGIN_LOG) << "Received" << fetch->count() << "events for collection" << col.id();
    });
}

void EventModel::onItemsReceived(const Akonadi::Item::List &items)
{
    qCDebug(THUNDERBIRDEVENTSPLUGIN_LOG) << "Batch: received" << items.count() << "items";
...
void EventModel::removeCollection(const Akonadi::Collection &col)
{
    ...
    qCDebug(THUNDERBIRDEVENTSPLUGIN_LOG) << "Removing" << items.count() << "events for collection" << col.id();
    if (items.isEmpty()) {
        return;
    }
